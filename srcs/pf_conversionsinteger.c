...
/*   Updated: 2024/05/28 23:45:24 by iwillens         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	pf_convert_integer_itoa(t_content *cnt, short type, size_t base)
{
	if (cnt->length == 0)
		...
		cnt->prt_v =
		ft_itoa_base((t_lli)(*(t_ptrdiff*)(cnt->value) & M_LLI), base, type);
}
...
void		pf_convert_integer(t_content *cnt, short type, size_t base)
{
	...
	if (cnt->prec.nb > 0)
		cnt->prt_v = pf_fillnchr_start(cnt->prt_v, '0',
			cnt->prec.nb);
...
static void	pf_convert_integer_uitoa(t_content *cnt, short type, size_t base)
{
	if (cnt->length == 0)
		...
		cnt->prt_v =
		ft_itoa_base(*(t_ptrdiff*)(cnt->value) & M_ULLI, base, type);
}
