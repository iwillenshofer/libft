...
void	pf_flags_add_sharp(t_content *cnt)
{
	...
	{
		cnt->prt_v = pf_insertchr_start(cnt->prt_v, 'x');
		cnt->prt_v = pf_insertchr_start(cnt->prt_v, '0');
	}
	else if (cnt->type == 'X')
	{
		cnt->prt_v = pf_insertchr_start(cnt->prt_v, 'X');
...
{
	if (cnt->prt_v[0] != '-')
		cnt->prt_v = pf_insertchr_start(cnt->prt_v, '+');
}
...
void	pf_flags_add(t_content *cnt)
{
	if (cnt->width.nb < 0)
	{
		...
		cnt->width.nb = pf_negative_topositive(cnt->width.nb);
