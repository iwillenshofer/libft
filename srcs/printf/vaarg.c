...
/*   Updated: 2024/06/07 16:59:56 by iwillens         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_values(va_list *ap, t_content *cnt)
{
	...
	if (ft_strchr(PF_SPEC_INT, cnt->type) && cnt->modifier == LM_LONG)
		cnt->value.u = va_arg(*ap, long);
	else if (ft_strchr(PF_SPEC_INT, cnt->type) && cnt->modifier == LM_SHORT)
		cnt->value.u = (short)va_arg(*ap, int);
	else if (ft_strchr(PF_SPEC_INT, cnt->type))
		cnt->value.u = va_arg(*ap, int);
	else if (ft_strchr(PF_SPEC_PTR, cnt->type))
		cnt->value.u = (unsigned long long)va_arg(*ap, void *);
	else if (ft_strchr(PF_SPEC_UINT, cnt->type) && cnt->modifier == LM_LONG)
		cnt->value.u = va_arg(*ap, unsigned long);
	else if (ft_strchr(PF_SPEC_UINT, cnt->type) && cnt->modifier == LM_SHORT)
		cnt->value.u = (unsigned short)va_arg(*ap, unsigned int);
